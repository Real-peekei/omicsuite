...
[![pkgdown
site](https://img.shields.io/badge/docs-pkgdown-3B6E8F)](https://real-peekei.github.io/omicsuite/)

An extensible collection of analytical pipelines for biostatistics,
bioinformatics, and computational biology. Every pipeline follows the
same contract: fit a model, run the diagnostic checks a careful methods
section would include, render `ggplot2` plots, and return a structured
verdict table – assumption checks, informational notes, and a
plain-language interpretation of the fitted effect – rather than a bare
point estimate. Verdict rows that explain a specific plot say so via a
`plot` column, so you always know which figure a given row is about –
plots themselves stay visually clean, with no embedded text.
...
[`ROADMAP.md`](https://real-peekei.github.io/omicsuite/ROADMAP.md) for
what’s next – omicsuite is meant to grow past these; see
[`CONTRIBUTING.md`](https://real-peekei.github.io/omicsuite/CONTRIBUTING.md)
for the shared pipeline contract new modules follow. The [package
website](https://real-peekei.github.io/omicsuite/) has the full
...
| Proportional hazards | [`survival::cox.zph()`](https://rdrr.io/pkg/survival/man/cox.zph.html), global + per-term | `fit$ph_test`, `fit$plots$ph_plot` |
...
All of the above are also summarized in `fit$verdicts`, a data.frame
with one row per check, a pass/flagged/review status, the relevant
statistic, and a plain-language note – written to be pasted directly
...
| Subdistribution effect | Fine-Gray model ([`cmprsk::crr()`](https://rdrr.io/pkg/cmprsk/man/crr.html)) | `fit$crr_fit`, `fit$verdicts` |
| Cause-specific vs. subdistribution | Standing interpretive note | `fit$verdicts` (`subdistribution_vs_cause_specific`) |

## Development notes

This package was scaffolded without a local R installation available, so
the `NAMESPACE` was hand-authored to match expected `roxygen2` output.
Before your first `R CMD check` or `devtools::check()`, run:
...
| Posterior predictive fit | [`brms::pp_check()`](https://mc-stan.org/bayesplot/reference/pp_check.html) overlay (visual review) | `fit$plots$pp_check` |
...
| Variance explained | Per-block AVE from [`RGCCA::rgcca()`](https://rgcca-factory.github.io/RGCCA/reference/rgcca.html) | `fit$variance_explained`, `fit$plots$variance_explained` |
